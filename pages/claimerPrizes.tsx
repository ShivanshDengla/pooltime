      ...
      await Promise.all(
        chainNames.map(async (chainName) => {
          try {
            ...
            // Last awarded draw (prizes are only claimable for this draw)
            ...
            // Check if finalized (already past the claim window).
            // If finalized, prizes can no longer be claimed.
            try {
              const finalized: boolean = await prizePool.isDrawFinalized(lastDrawId);
              if (finalized) return;
            } catch {
              // fall through — attempt anyway
            }

            const winsUrl = `https://poolexplorer.xyz/${chainId}-${cfg.PRIZEPOOL.toLowerCase()}-draw${lastDrawId}`;
            const claimsUrl = `https://poolexplorer.xyz/claims-${chainId}-${cfg.PRIZEPOOL.toLowerCase()}-draw${lastDrawId}`;
            ...
            if (!winsRes || !winsRes.ok) {
              console.warn(`[${chainName}] no wins data for draw ${lastDrawId}`);
              return;
            }
            const winsData = await winsRes.json().catch(() => null);
            if (!winsData?.wins || !Array.isArray(winsData.wins)) return;
            ...
            groupArr.forEach((g, idx) => {
              ...
              const claimCount = prizeIndices.reduce((s, arr) => s + arr.length, 0);
              if (claimCount === 0) return;

              const prizeSize = tierInfo.prizeSize;
              const feePerClaim = (feePerClaimResults[idx] as ethers.BigNumber) ||
              ...
              const isCanary =
                numTiers > 0 && (g.tier === numTiers - 1 || g.tier === numTiers - 2);
              ...
              const assetSymbol = vaultInfo?.ASSETSYMBOL || vaultInfo?.SYMBOL || "";
              ...
              perChainResults.push({
                key: `${chainName}-${g.vault}-${g.tier}-${lastDrawId}`,
                chainName,
                chainId,
